...
if prompt := st.chat_input("How can i help you today?"):
    ...
    with st.chat_message("assistant"):
        response = bedrockclient.orchestrate_conversation(prompt,st.session_state.messages, st.session_state.session_id)
        print(response)
        st.markdown(response)
        content_asst = {"role":"assistant", "content": [{"text" : response}]}
        #st.markdown(content['text'])
